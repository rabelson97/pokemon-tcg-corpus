...
def retrieval_at_1(model: CardEmbeddingModel, dataloader: DataLoader, device: torch.device) -> float:
    model.eval()
    embeddings: list[torch.Tensor] = []
    card_ids: list[str] = []
    with torch.no_grad():
        for images, _, batch_ids in dataloader:
    ...
    correct = sum(1 for idx, match_idx in enumerate(nearest.tolist()) if card_ids[idx] == card_ids[match_idx])
    return correct / max(1, len(card_ids))
...
def main() -> int:
    ...
    val_loader = DataLoader(
        TwoViewCardDataset(val_records, build_eval_transform(args.image_size)),
        batch_size=args.batch_size,
        shuffle=False,
        num_workers=2,
        pin_memory=torch.cuda.is_available(),
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        val_recall = retrieval_at_1(model, val_loader, device)
        avg_loss = running_loss / max(1, len(train_loader))
        print(f"epoch={epoch} loss={avg_loss:.4f} val_recall_at_1={val_recall:.4f}")
